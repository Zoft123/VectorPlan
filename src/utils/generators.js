export const generateYaml = (entities, paths) => {
  ...
  // Filter out ONLY rooms from the YAML, so structural elements are included
...
export const generateSvg = (entities, paths, canvasSize) => {
  ...
  if (structural.length > 0) {
    ...
    structural.forEach(ent => {
      if (ent.kind === 'Window') {
        ...
        svg += `    <line x1="${ent.width/2}" y1="${-ent.depth/2}" x2="${ent.width/2}" y2="${ent.depth/2}" stroke="${ent.color}" stroke-width="2" />\n`;
        svg += `    <line x1="${-ent.width/2}" y1="0" x2="${ent.width/2}" y2="0" stroke="${ent.color}" stroke-width="1" stroke-dasharray="2 4" opacity="0.5" />\n`;
        svg += `    <g class="door-moving-part" style="transform-origin: ${-ent.width/2}px 0px; --door-swing: ${ent.flip ? '-90deg' : '90deg'};">\n`;
        svg += `      <line x1="${-ent.width/2}" y1="0" x2="${-ent.width/2}" y2="${ent.flip ? ent.width : -ent.width}" stroke="#d97706" stroke-width="4" stroke-linecap="round" />\n`;
        svg += `      <path d="M ${-ent.width/2},${ent.flip ? ent.width : -ent.width} A ${ent.width} ${ent.width} 0 0 ${ent.flip ? 0 : 1} ${ent.width/2},0" fill="none" stroke="${ent.color}" stroke-width="1" stroke-dasharray="4 4" opacity="0.6" class="door-arc" />\n`;
        svg += `    </g>\n`;
  ...
  entities.filter(e => !['Room', 'Door', 'Window'].includes(e.kind)).forEach(ent => {
    if (ent.kind === 'Light') {
      ...
      svg += `      <polygon points="44,44.5 56,44.5 56,47 44,47" fill="#1C5D82" stroke="#003855" stroke-width="1.5" stroke-linejoin="round"/>\n`;
      svg += `      <polygon points="59,44.5 71,44.5 74,47 58,47" fill="#1C5D82" stroke="#003855" stroke-width="1.5" stroke-linejoin="round"/>\n`;
      svg += `      <polygon points="22,48 78,48 81,52 19,52" fill="white" stroke="#003855" stroke-width="2" stroke-linejoin="round"/>\n`;
      svg += `      <polygon points="19,52 81,52 83,56 17,56" fill="white" stroke="#003855" stroke-width="2" stroke-linejoin="round"/>\n`;
      svg += `      <polygon points="17,56 83,56 83,60 17,60" fill="white" stroke="#003855" stroke-width="2" stroke-linejoin="round"/>\n`;
      svg += `    </svg>\n`;
      svg += `    <circle cx="${ent.x}" cy="${ent.y}" r="${ent.radius}" class="entity-hitbox" />\n  </g>\n`;
    } else {
