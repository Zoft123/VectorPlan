export const generateYaml = (entities, paths) => {
  ...
  entities.filter(e => e.kind !== 'Room').forEach(ent => {
    ...
    // Extra rule specifically for Thermostat Text
...
export const generateSvg = (entities, paths, canvasSize) => {
  ...
  if (rooms.length > 0) {
    svg += `  <!-- Room Areas -->\n`;
    rooms.forEach(room => {
      svg += `  <polygon id="${room.roomId}" points="${room.points.map(p => `${p.x},${p.y}`).join(' ')}" class="room-area" style="--room-fill: ${room.fillColor || '#ffffff'}; --room-opacity: ${(room.opacity ?? 40) / 100};" />\n`;
    });
    svg += `\n`;
  }
